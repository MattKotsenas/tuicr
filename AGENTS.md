...
7. **Reload diff/session**: `:e` reloads persisted comments/review state, then re-runs VCS diff loading and reapplies `.tuicrignore` filtering to refresh displayed files. Full reloads persist only their derived file hashes and invalidated review markers, without saving unrelated dirty edits.
